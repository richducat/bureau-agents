...
export default function PaymentSettingsPage() {
  ...
  const subscribe = async (organizationId: string, plan: 'operator_pro' | 'client_scale') => {
    if (!readiness.acceptingNewPayments) {
      setError('Paid plans are not activated during the founding beta. Your free account and work requests remain available.')
      return
    }
    setWorking(true); setError(''); track('plan_selected', { plan })
    try {
    ...
    } catch (caught) { setError(caught instanceof ApiError ? caught.message : 'Subscription checkout could not be opened.') }
    finally { setWorking(false) }
  }
  ...
  return <div className="payment-settings"><header className="page-heading"><div><p className="overline">Money controls</p><h1>Billing & payouts</h1><p>Stripe handles payment credentials, identity verification, bank connections, invoices, and payouts.</p></div></header>{!readiness.acceptingNewPayments && <div className="commercial-payment-callout" role="status"><ShieldCheck /><div><strong>Founding-beta payment hold</strong><p>Account setup and payout verification are open. New task funding, paid verification, and plan upgrades stay disabled until Bureau completes its launch approvals.</p></div></div>}{connectResult === 'return' && <div className="success-message" role="status">Returned from Stripe. Bureau refreshed the payout status below.</div>}{connectResult === 'refresh' && <div className="form-error" role="status">The Stripe verification link expired or needs another step. Continue verification below.</div>}{error && <p className="form-error" role="alert">{error}</p>}<section className="payment-cards">{client && <article><span><CreditCard /></span><p className="overline">Client billing</p><h2>{client.name}</h2><p>Current plan: <strong>{client.plan === 'client_scale' ? 'Scale · 3%' : 'Starter · 5%'}</strong></p><div>{client.plan === 'client_scale' ? <button className="button button--secondary" disabled={working} onClick={() => void portal(client.id)}>Manage subscription <ArrowUpRight /></button> : <button className="button button--dark" disabled={working || !readiness.acceptingNewPayments} onClick={() => void subscribe(client.id, 'client_scale')}>{readiness.acceptingNewPayments ? 'Upgrade to Scale · $149/mo' : 'Scale opens after founding beta'}</button>}</div></article>}{operator && <article><span><Landmark /></span><p className="overline">Operator payouts</p><h2>{operator.name}</h2><div className={`payout-readiness ${connectStatus?.payoutsEnabled ? 'is-ready' : ''}`}>{statusLoading ? <LoaderCircle className="spin" /> : connectStatus?.payoutsEnabled ? <CheckCircle2 /> : <TriangleAlert />}<div><strong>{statusLoading ? 'Checking Stripe…' : statusLabel}</strong><small>{connectStatus?.payoutsEnabled ? 'This operator can receive released milestone funds after paid work opens.' : currentlyDue ? `${currentlyDue} Stripe ${currentlyDue === 1 ? 'requirement is' : 'requirements are'} currently due.` : 'Complete Stripe verification now so the operator is ready when paid work opens.'}</small></div><button className="icon-button" aria-label="Refresh payout status" disabled={statusLoading} onClick={() => void refreshConnectStatus()}><RefreshCcw /></button></div><div><button className="button button--dark" disabled={working || statusLoading} onClick={() => void connect()}>{connectStatus?.payoutsEnabled ? 'Review Stripe account' : connectStatus?.connected ? 'Continue Stripe verification' : 'Verify payouts with Stripe'} <ArrowUpRight /></button>{operator.plan === 'operator_pro' ? <button className="button button--secondary" disabled={working} onClick={() => void portal(operator.id)}>Manage Pro</button> : <button className="button button--secondary" disabled={working || !readiness.acceptingNewPayments} onClick={() => void subscribe(operator.id, 'operator_pro')}>{readiness.acceptingNewPayments ? 'Operator Pro · $49/mo' : 'Pro opens after founding beta'}</button>}</div></article>}</section><section className="payment-safety"><ShieldCheck /><div><h2>What Bureau stores</h2><p>Bureau stores Stripe object identifiers, fee calculations, payment state, and an audit ledger. Raw card and bank credentials never enter the Bureau application database.</p></div><Link to="/payment-protection">Read payment protection</Link></section></div>
}
