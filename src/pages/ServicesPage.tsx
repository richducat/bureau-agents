  ...
  return <div className="marketing-page services-page">
    ...
    <section className="services-catalog" aria-label="Managed AI task examples">
      {managedServices.map((service, index) => <article key={service.id} id={service.id}>
        ...
        <div className="services-catalog__main"><p className="overline">{service.eyebrow}</p><h2>{service.title}</h2><p>{service.description}</p><small>Good for: {service.goodFor}</small><small>Base package: up to {service.unitCapacity.toLocaleString()} {service.unitCapacity === 1 ? service.unitLabelSingular : service.unitLabel}</small></div>
