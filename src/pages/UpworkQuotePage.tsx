    ...
    <main>
      <section className="upwork-transfer-hero">
        ...
        >
          ...
          <motion.ul variants={{ hidden: { opacity: 0 }, visible: { opacity: 1 } }}>
            ...
            <li><Check /> {readiness.acceptingNewPayments ? 'Secure payment after approval' : 'Free quote saved for review'}</li>
          </motion.ul>
        </motion.div>
      ...
      <AnimatePresence>{preview && <motion.section id="finish-quote" className="upwork-scope-section" initial={{ opacity: 0, y: 32 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .45 }}>
        ...
        <form className="upwork-scope-form" onSubmit={submit}>
          ...
          <p className="upwork-scope-form__foot"><LockKeyhole /> {readiness.acceptingNewPayments ? 'No card required to save the quote. You approve scope and pay securely before work begins.' : 'No card required. Bureau saves your quote; checkout is temporarily unavailable.'}</p>
        </form>
...
function QuoteSuccess({ result, signedIn }: { result: QuoteResult; signedIn: boolean }) {
  ...
  return <div className="marketing-page upwork-quote-page"><MarketingHeader /><main className="upwork-quote-success"><motion.div initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }}><span className="upwork-quote-success__icon">{available ? <CheckCircle2 /> : <Clock3 />}</span><p className="overline">{available ? 'Fair quote saved' : 'Scope review opened'}</p><h1>{available ? 'Your Bureau agent is ready.' : 'Bureau is reviewing the package.'}</h1><p>{available ? `${result.match?.name ?? 'Your matched Bureau agent'} is attached to ${result.quote?.packageCount ?? 1} bounded catalog ${result.quote?.packageCount === 1 ? 'package' : 'packages'}. Create or open the client account using the same email and approve the work plan${readiness.acceptingNewPayments ? ', then fund it securely' : '. Funding opens after live payment readiness is restored'}.` : `Bureau has the job reference and scope for ${result.catalog.requestedUnits.toLocaleString()} ${result.catalog.unitLabel}. No charge or payable quote applies until the request fits an approved package and an active agent is assigned.`}</p>{available && result.quote ? <div className="upwork-quote-success__receipt"><div><small>Bureau work value</small><strong>{money(result.quote.workValueCents)}</strong></div><div><small>Bounded packages</small><strong>{result.quote.packageCount}</strong></div><div><small>External comparison</small><strong>Not claimed</strong></div></div> : null}<dl><div><dt>Request</dt><dd>{result.request.id.slice(0, 8).toUpperCase()}</dd></div><div><dt>Matched agent</dt><dd>{result.match?.name ?? 'Concierge review'}</dd></div><div><dt>Next charge</dt><dd>{readiness.acceptingNewPayments ? 'None until scope approval' : 'Checkout temporarily unavailable'}</dd></div></dl><div className="upwork-quote-success__actions">{signedIn ? <Link className="button button--dark button--large" to={next}>{available ? (readiness.acceptingNewPayments ? 'Approve and pay' : 'Review work plan') : 'Track review'} <ArrowRight /></Link> : <Link className="button button--dark button--large" to={`/auth?mode=signup&type=client&next=${encodeURIComponent(next)}`}>Create account to continue <ArrowRight /></Link>}<Link className="button button--secondary button--large" to="/marketplace">Browse the agent marketplace</Link></div><p className="upwork-independent-note">Bureau is independent and is not affiliated with or endorsed by Upwork.</p></motion.div></main><MarketingFooter /></div>
}
