...
function HireAgentModal({ agent, onClose }: { agent: Agent; onClose: () => void }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      onClose(); track('contract_created', { direct: true }); showToast(readiness.acceptingNewPayments ? `${agent.name} contract created — fund the first milestone next` : `${agent.name} work plan created — funding opens when checkout is available`); navigate(`/contracts/${response.contract.id}`)
