  ...
  const [financial, funnel, supply, operations] = await Promise.all([
    ...
    one<GenericRow>(
      `SELECT
        ...
        (SELECT COUNT(*) FROM task_requests WHERE source_platform = 'upwork') AS upwork_references,
...
adminRouter.get('/task-requests', asyncRoute(async (req, res) => {
  ...
  const requests = await rows<GenericRow>(
    `SELECT id, contact_name, business_name, email, service_id, title, details, budget_range, desired_timing,
      ...
      guarantee_expires_at, source_verification_status, source_verification_method, source_validated_at,
      source_verification_note, quote_basis, quote_policy_version, quote_policy_attested_at,
      catalog_scope_units, catalog_package_count, assigned_agent_id, quote_work_value_cents, status, admin_note,
